...
def get_urlandcookie():
    ...
    file_path = os.path.join(os.path.dirname(__file__), "wayfair_TOTO_url.txt")
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
...
def get():
    ...
    for data in sql_data:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT 目前售价, 本次更新review数量 FROM wayfair_TOTO WHERE 商品名称 = ?", (data['商品名称'],))
        row = cursor.fetchone()
        if row:
            print("目前售价:", row[0])
            print("本次更新review数量:", row[1])
            cursor.execute("UPDATE wayfair_TOTO SET 目前售价 = ? WHERE 商品名称 = ?", (data['目前售价'], data['商品名称']))
            cursor.execute("UPDATE wayfair_TOTO SET 本次更新review数量 = ? WHERE 商品名称 = ?", (data['评论数量'], data['商品名称']))
            cursor.execute("UPDATE wayfair_TOTO SET 上次同步前售价 = ? WHERE 商品名称 = ?", (row[0], data['商品名称']))
            cursor.execute("UPDATE wayfair_TOTO SET 上次同步时数量 = ? WHERE 商品名称 = ?", (row[1], data['商品名称']))
            cursor.execute("UPDATE wayfair_TOTO SET 更新时间 = ? WHERE 商品名称 = ?", (data['更新时间'], data['商品名称']))
        else:
            insert_query = ("INSERT INTO wayfair_TOTO (sku,主图, 目前售价, 上次同步前售价, 本次更新review数量, 上次同步时数量, 更新时间, 直达的落地页, 产品评分, 商品名称) "
                            "VALUES (?,?, ?, '', ?, '', ?, ?, ?, ?)")
