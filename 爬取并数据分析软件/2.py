...
a = ['https://www.amazon.com//-/zh/dp/B07TDSYY2D/ref=sr_1_13?keywords=Swiss+Madison&qid=1690698117&sr=8-13','https://www.amazon.com//-/zh/dp/B07VHQ2MKK/ref=sr_1_12?keywords=Swiss+Madison&qid=1690698117&sr=8-12']
while len(a) > 0:
    url = a[0].strip()
    # 打开网站
    ...
    try:
        mingcheng = soup.find('span', class_='a-size-large product-title-word-break').get_text()
        data["商品名称"] = mingcheng
    except:
        try:
            mingcheng = soup.find('span', {'class': ['a-size-large', 'product-title-word-break']})
            data["商品名称"] = mingcheng
        except:
            mingcheng = "无名称" + url
            a.pop(0)
